	...
	/**
	 ...
	 public String doKMLExport(String queryParameter, KMLExportOptions exportOptions) throws javax.servlet.ServletException {
		...
		try {
			...
			exportFile.addAuthorInfo(firstFolder, dataManager.getContextParam("systemName"), dataManager.getContextParam("systemUrl"));
				
			// add a description element to this folder
			exportFile.addDescription(firstFolder, "Maps of events by contributor exported from the AusStage system on " + getCurrentDate() + ".");
			...
			for(int i = 0; i < ids.length; i++) {
				...
				try {
					...
					exportFile.addDescription(contribFolder, "Maps of events associated with: " + currentContributor);
					
					// add a document for the standard map
					Element document = exportFile.addDocument(contribFolder, "Events");
					exportFile.addDescription(document, "One place marker for each event");
