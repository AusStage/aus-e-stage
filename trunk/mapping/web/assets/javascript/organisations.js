...
var organisationMapData = null
var organisationIDs = null;
...
function showOrganisationMap(id, contrib, url) {
	...
	$("#map_header_link").attr("href", "maplinks.jsp?type=organisation&id=" + id);
	
	// update the download as KML link
	$("#map_header_kml").attr("href", "data?action=kml&type=organisation&id=" + id);
	...
	$.get("data?action=markers&type=organisation&id=" + id + '&rnd=' + rnd.toString(), null, function(data) {
...
function reloadMap() {
	...
	if(showTraj != null) {
		...
		showMap(organisationMapData, true, $("#state").val(), startDate, finishDate);
	} else {
		// reload the map with trajectory information
		showMap(organisationMapData, false, $("#state").val(), startDate, finishDate);
	}
}
...
function loadNewPage(responseText, statusText) {
	window.location = "maplinks.jsp?type=organisation&id=" + $("#id").val();
}
